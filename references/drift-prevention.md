...
When you change a source of truth, update its dependent files in the same change. See `AGENTS.md` for automated enforcement (pre-commit hook and CI run validate-docs.py, which checks stale patterns, cross-references, script/domain/section/snippet inventory, skill essentials, and pipe artifacts).
...
| `config.el` | `sections/*.el`, `PROFILE.md` custom functions table, `DOOM-API.md` patterns, `README.md` File Layout |
| `config.el` / `sections/*.el` | Re-evaluate `.agents/` files for stale examples, paths, procedures |
| `DOOM-API.md` | `.agents/skills/doom-emacs/SKILL.md` Doom API Essentials, `domains/PROCEDURES.md` sections C, D, E |
| `sections/*.el` (user/ functions) | `PROFILE.md` custom functions table |
